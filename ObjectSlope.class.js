var ObjectSlope = function() {
	...
	that.collide = function( obj, b ) { 
		...
		if(obj.speed_y < 0 && obj.y > (b.y + b.heightMap[indexHeightMap]) ) {
			console.log("falsing");
			return false;
		}
		...
		// 	 		return false;
		// } else {
			if(obj.y > b.y + b.heightMap[indexHeightMap]) {
		...
		if(obj.speed_y >= 0) { 
			...
			obj.angle = b.angleMap[floor(indexHeightMap/32)];

			obj.y = b.y + b.heightMap[indexHeightMap] - obj.sm.currentState.frames[floor(obj.frame)].height;
	...
	that.initSensors = function() {
		...
		for(var i=0; i<that.heightMap.length; i++) {
			...
			sensor.update = function(x, y, width, height) {
				sensor.x = x + i;
				sensor.y = y + that.heightMap[i] + 12;
				sensor.width = 1; 
				sensor.height = 128;
			};
	...
	that.flip = function() {
		...
		that.heightMap = flipArrayHorizontal(that.heightMap);
		that.angleMap = flipArrayHorizontal(that.angleMap);

		for(var a=0; a<that.angleMap.length;a++) {
			that.angleMap[a] = (-1) * that.angleMap[a];
		}
